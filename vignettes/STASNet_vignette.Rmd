...
Now that we have a satisfying model, it can be exported with `exportModel`. Note that the model *cannot* be saved as an RData object because of the underlying C++ code, it has to be saved with `exportModel`.
```{r}
exportModel(red_model, "best_model.mra")
```
The model can then be rebuilt with `importModel` (or with the more complex `rebuildModel` which allows to retrain the model with new data).
```{r}
red_model = importModel("best_model.mra")
```
